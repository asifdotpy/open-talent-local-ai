...
def test_integration():
    ...
    try:
        tts_response = requests.post(f"{VOICE_SERVICE_URL}/voice/tts", json=tts_request, timeout=30)
    ...
    if phonemes_from_tts and len(phonemes_from_tts) > 0:
        ...
        print("✅ Using phonemes from voice service")
    else:
        print("⚠️  No phonemes from TTS, using sample data")
        phonemes = [
    ...
    try:
        ...
        render_response = requests.post(
            f"{AVATAR_SERVICE_URL}/render/lipsync", json=render_request, timeout=60
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("⚠️  Could not verify video (ffprobe not available)")
    ...
    print("🎯 Complete flow: Voice TTS → Phonemes → Avatar Video")
    return True
