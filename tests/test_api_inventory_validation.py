...
async def get_openapi_schema(client: httpx.AsyncClient, base_url: str) -> dict | None:
    ...
    async def test_openapi_info_complete(self, service_key: str, service_config: dict):
        ...
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            try:
                ...
                assert "title" in info, f"{service_config['name']}: OpenAPI info missing 'title'"
...
class TestServiceIntegration:
    ...
    async def test_all_services_runnable(self):
        ...
        assert len(running_services) > 0, f"No core services running. Started: {running_services}"
    ...
    async def test_notification_service_complete(self):
        ...
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            try:
                ...
                schema_response = await client.get(f"{base_url}/openapi.json")
                assert schema_response.status_code == 200, (
                    "Notification Service OpenAPI schema unavailable"
                )

                schema = schema_response.json()
                assert len(schema.get("paths", {})) > 0, "Notification Service OpenAPI has no paths"
...
class TestSummaryReport:
    ...
    async def test_generate_service_summary(self, capsys):
        ...
        for service_key, info in summary["services"].items():
            ...
            print(
                f"{status} {info['name']:40s} | Port: {info['port']} | OpenAPI: {openapi} | Endpoints: {info['endpoints_count']}"
            )
