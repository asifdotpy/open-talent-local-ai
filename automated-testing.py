...
results = {"timestamp": datetime.now().isoformat(), "model": "granite4:350m-h", "tests": []}


def check_ollama():
    """Check if Ollama is available and has granite4:350m-h"""
    try:
        response = subprocess.run(["curl", "-s", OLLAMA_LIST], capture_output=True, text=True)
...
def test_role(role_config, role_index):
    ...
    try:
        ...
        for i, question in enumerate(role_config["questions"]):
            ...
            payload = {"model": "granite4:350m-h", "messages": conversation, "stream": False}
...
def save_results():
    ...
    for idx, test in enumerate(results["tests"]):
        ...
        diagnosis_content += (
            f"- First response time: {test['questions'][0]['responseTime'] / 1000:.2f}s\n"
        )
        ...
        diagnosis_content += (
            "- Notes: All questions tested, quality measured by technical depth and coherence\n\n"
        )
    ...
    avg_subsequent = (
        sum(q["responseTime"] for t in results["tests"] for q in t["questions"][1:])
        / sum(len(t["questions"]) - 1 for t in results["tests"])
        / 1000
    )
