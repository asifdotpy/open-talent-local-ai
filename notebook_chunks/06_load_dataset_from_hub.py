...
try:
    ...
    print("🔄 Attempting fallback: Create minimal dataset...")
    ...
    print("✅ Created fallback dataset:")
    print(f"   Train: {len(dataset_dict['train'])} examples")
    print(f"   Validation: {len(dataset_dict['validation'])} examples")
    print("\n⚠️  NOTE: Using minimal fallback. For full training, ensure:")
    print("   1. HF token is valid (has access to private dataset)")
    print("   2. Dataset is pushed to: asifdotpy/OpenTalent-comprehensive-dataset")
    print("   3. Token user has read access to private repos")
