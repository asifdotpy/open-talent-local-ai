...
class OpenAPIValidator:
        ...
        if openapi_version not in self.OPENAPI_3_VERSIONS:
            if openapi_version.startswith("3."):
                ...
                self.result.errors.append(ValidationError(
                    ...
                    suggestion=f"Use OpenAPI 3.x (e.g., '3.0.0' or '3.1.0')"
                ))
    ...
    def _validate_info(self):
        ...
        if "contact" not in info:
            self.result.info.append(ValidationError(
                level="info",
                path="$.info",
        ...
        for path, path_item in paths.items():
            if not isinstance(path_item, dict):
                self.result.errors.append(ValidationError(
                    level="error",
                    path=f"$.paths.{path}",
                    message=f"Path item must be an object/dict"
                ))
                continue
...
async def fetch_openapi_schema(service_port: int, timeout: int = 5) -> Optional[Dict]:
...
async def validate_service(service_name: str, verbose: bool = False) -> Optional[ValidationResult]:
    """Validate specific service"""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Validate OpenAPI schemas for all microservices"
    )
