...
run_integration() {
    ...
    if ! curl -f http://localhost:8015/health &>/dev/null; then
        echo -e "${YELLOW}Warning: Voice service not running on port 8015${NC}"
        echo "Start with: docker-compose up -d voice-service"
        echo "Or run locally: python -m uvicorn app.main:app --port 8015"
...
run_business() {
    ...
    if ! curl -f http://localhost:8015/health &>/dev/null; then
        echo -e "${RED}✗ Voice service not running (port 8015)${NC}"
        services_ok=false
    else
