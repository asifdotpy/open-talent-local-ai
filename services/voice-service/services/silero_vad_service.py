...
class SileroVADService:
    ...
    def detect_voice_activity(self, audio_file_path: str) -> dict:
        """Analyze an audio file to identify segments containing active human speech.

        Args:
            audio_file_path: Absolute path to the audio file to analyze.

        Returns:
            A dictionary containing speech segment timestamps, total speech/silence
            durations, and the speech-to-audio ratio.
        """
        try:
            # Read audio file
    ...
    def filter_silence(self, audio_file_path: str, output_path: str) -> dict:
        """Process an audio file to remove all detected silent intervals.

        Args:
            audio_file_path: Absolute path to the source audio file.
            output_path: Absolute path where the processed audio (void of silence) will be saved.

        Returns:
            A dictionary summarizing the reduction in duration and the path to the output file.
        """
        try:
            # Read audio
