...
class VoskSTTService:
    ...
    def __init__(
        self,
        model_path: str = DEFAULT_VOSK_MODEL_PATH,
    ...
    def load_model(self):
        """Load the Vosk model from the filesystem into memory.
    ...
    def transcribe_audio(self, audio_file_path: str) -> dict:
        """Transcribe an audio file into text with word-level timing and confidence scores.

        Automatically handles mono/stereo conversion and resampling to the model's required sample rate.

        Args:
            audio_file_path: Absolute path to the audio file (WAV, MP3, etc.).

        Returns:
            A dictionary containing the full text, word-level metadata, duration, and confidence.
        """
        try:
    ...
    def transcribe_streaming(self, audio_chunk: bytes) -> dict | None:
        """Process a small buffer of audio data for real-time streaming.

        Args:
            audio_chunk: Raw bytes of 16-bit PCM audio data.

        Returns:
            A dictionary containing partial or final results if a complete
            utterance is detected, otherwise None.
        """
        try:
    ...
    def reset_recognizer(self):
        """Reset the recognizer state to begin a new transcription session."""
        if self.recognizer:
    ...
    def _resample_audio(self, audio: np.ndarray, orig_sr: int, target_sr: int) -> np.ndarray:
        ...
        Args:
            ...
            orig_sr: The source sample rate in Hz.
            target_sr: The desired target sample rate in Hz.

        Returns:
            The resampled audio signal as a NumPy array.
        """
    ...
    def health_check(self) -> bool:
        """Check if the Vosk service is initialized and ready for processing.

        Returns:
            bool: True if both model and recognizer are loaded, False otherwise.
        """
