...
async def register(request: Request, payload: RegisterRequest = Body(...)):
    ...
    if not is_strong_password(password):
        return JSONResponse(
            status_code=422,
            content={"error": "Password too weak. Must contain uppercase, lowercase, digits, and special characters"}
        )
...
async def refresh(payload: TokenRefreshRequest = Body(...)):
    ...
    if not refresh_token:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing refresh token"}
        )
...
async def verify_mfa(payload: MFAVerifyRequest = Body(...), current_user: str | None = Depends(get_current_user)):
    """Verify MFA code"""
    email = current_user
    ...
    if not code:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing MFA code"}
        )
    ...
    if user.get("mfa_secret") and len(code) == 6 and code.isdigit():
        ...
        return JSONResponse(
            status_code=200,
            content={"verified": True, "message": "MFA verified successfully"}
        )
...
async def check_permission(payload: PermissionCheckRequest = Body(...), current_user: str | None = Depends(get_current_user)):
    ...
    return JSONResponse(
        status_code=200,
        content={"permission": permission, "has_permission": has_permission}
    )
...
async def encrypt_data(payload: EncryptRequest = Body(...)):
    ...
    if not data:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing data to encrypt"}
        )
...
async def decrypt_data(payload: DecryptRequest = Body(...)):
    ...
    if not encrypted:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing encrypted data"}
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "Failed to decrypt data"}
        )
...
@limiter.limit(RATE_LIMIT_RULE) if limiter else (lambda f: f)
async def change_password(request: Request, payload: ChangePasswordRequest = Body(...), current_user: str | None = Depends(get_current_user)):
    ...
    if not current_password or not new_password:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing current or new password"}
        )
    ...
    if not is_strong_password(new_password):
        return JSONResponse(
            status_code=422,
            content={"error": "New password too weak"}
        )
...
async def reset_password(request: Request, payload: PasswordResetWithTokenRequest = Body(...)):
    ...
    if not token or not new_password:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing token or new password"}
        )
...
async def assign_role(payload: AssignRoleRequest = Body(...), current_user: str | None = Depends(get_current_user)):
    """Assign role to user"""
    email = current_user
    ...
    if not user_id or not role:
        return JSONResponse(
            status_code=400,
            content={"error": "Missing user_id or role"}
        )
    ...
    if email not in users_db:
        return JSONResponse(
            status_code=404,
            content={"error": "Current user not found"}
        )
    ...
    if "admin" not in current_user["roles"]:
        return JSONResponse(
            status_code=403,
            content={"error": "Only admins can assign roles"}
        )
    ...
    if user_id in users_db:
        ...
        return JSONResponse(
            status_code=200,
            content={"message": f"Role {role} assigned to user"}
        )
...
async def revoke_role(payload: RevokeRoleRequest = Body(...), current_user: str | None = Depends(get_current_user)):
    ...
    if "admin" not in current_user["roles"]:
        return JSONResponse(
            status_code=403,
            content={"error": "Only admins can revoke roles"}
        )
    ...
    if user_id in users_db:
        ...
        return JSONResponse(
            status_code=200,
            content={"message": f"Role {role} revoked from user"}
        )
