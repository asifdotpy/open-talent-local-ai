...
# Default to mock mode for local/dev and tests
...
def generate_questions_from_ollama(
    ...
    """
    # Short-circuit to mock generation when mock mode is enabled
    if USE_MOCK:
...
"""
    ...
    # Run the async function. If an event loop is already running (FastAPI handlers),
    # fall back to mock generation to avoid nested loop errors.
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running() and USE_MOCK:
            logger.info("Existing event loop detected; using mock questions for compatibility.")
            return _generate_mock_questions(job_description, num_questions, difficulty)
    except RuntimeError:
        loop = None

    try:
        if loop and loop.is_running() and USE_MOCK:
            return _generate_mock_questions(job_description, num_questions, difficulty)
