...
class TestInterviewAligned:
    def test_health_check(self, test_client_rooms: TestClient):
        response = test_client_rooms.get("/health")
        assert response.status_code == 200
        assert response.json()["status"] == "healthy"

    def test_root_endpoint(self, test_client_rooms: TestClient):
        response = test_client_rooms.get("/")
        assert response.status_code == 200
    ...
    def test_create_and_get_room(self, test_client_rooms: TestClient, create_room_data: dict):
        ...
        # Get Status
        response = test_client_rooms.get(f"/api/v1/rooms/{room_id}/status")
        assert response.status_code == 200
        assert response.json()["room_id"] == room_id

        # List Rooms
        response = test_client_rooms.get("/api/v1/rooms")
        assert response.status_code == 200
        assert any(r["room_id"] == room_id for r in response.json()["rooms"])

    def test_start_interview(self, test_client_rooms: TestClient, create_room_data: dict):
    ...
    def test_vetta_info(self, test_client_rooms: TestClient):
        response = test_client_rooms.get("/api/v1/vetta/info")
