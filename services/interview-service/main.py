...
app = FastAPI(
    title="TalentAI - Interview Service",
...
class ResponseAnalysis(BaseModel):
    ...
    followup_suggestions: List[FollowupQuestion] = Field(default_factory=list, description="Suggested follow-up questions")
...
async def root():
    ...
    return {
        "message": "TalentAI Interview Service - Room Infrastructure Management",
        "version": "1.0.0",
...
async def create_interview_room(request: CreateRoomRequest):
    ...
    try:
        ...
        existing_room = next((r for r in rooms_store.values() 
                            if r.interview_session_id == request.interview_session_id 
                            and r.status != RoomStatus.ENDED), None)
        if existing_room:
            logger.warning(f"Room already exists for session {request.interview_session_id}")
            raise HTTPException(status_code=409, detail=f"Room already exists for session {request.interview_session_id}")
...
async def get_room_status(room_id: str):
    ...
    if datetime.now() > room.expires_at and room.status not in [RoomStatus.ENDED, RoomStatus.EXPIRED]:
...
async def webrtc_signaling(
    ...
    connection = next((c for c in webrtc_connections.values() 
                      if c.room_id == room_id and c.participant_id == signal.participant_id), None)
    if not connection:
...
async def stop_webrtc_audio_stream(
    room_id: str,
    ...
    if participant_id:
        connections_to_stop = [c for c in webrtc_connections.values() 
...
async def broadcast_transcription_update(room_id: str, update: LiveTranscriptionUpdate):
    ...
    for connection in room_connections:
        try:
            ...
            logger.error(f"Message data types: segment.text={type(update.segment.text)}, timestamp={type(update.timestamp)}")
...
async def clear_transcription_history(room_id: str):
    ...
    return {
        "message": f"Cleared {segment_count} transcription segments",
        "room_id": room_id
    }
...
async def analyze_candidate_expertise(responses: List[InterviewAnswer]) -> str:
    ...
    technical_indicators = ["algorithm", "database", "api", "framework", "architecture", "scalability"]
...
async def generate_intelligence_report(
    ...
    responses: List[InterviewAnswer], 
    room: InterviewRoom
) -> IntelligenceReport:
    ...
    return IntelligenceReport(
        ...
        expertise_evaluation={
            ...
            "technical_skills_identified": list(set(skill for a in analyses for skill in a.expertise_assessment.technical_skills)),
            "average_experience_years": sum(a.expertise_assessment.experience_years or 0 for a in analyses) / len(analyses)
        },
        quality_metrics={
...
async def adapt_interview_strategy(
    room_id: str,
    request: InterviewAdaptationRequest
):
    """
    ...
    try:
        ...
        adaptations = await generate_interview_adaptations(
            ...
            request.time_remaining_minutes
        )
