...
class HealthMonitor:
    ...
    async def perform_health_check(self) -> HealthReport:
        """Perform a comprehensive health check of all registered agents.

        Triggers a fresh health check for all agents and aggregates the results
        into a detailed report.

        Returns:
            A HealthReport object with current status and metrics of all agents.
        """
    ...
    def _get_recent_checks(self, limit: int = 10) -> list[HealthCheckResult]:
        """Retrieve the most recent health check results from the history.

        Args:
            limit: Maximum number of recent checks to return (defaults to 10).

        Returns:
            A list of HealthCheckResult objects.
        """
    ...
    def get_agent_status(self, agent_name: str) -> AgentStatus | None:
        ...
        Returns:
            The AgentStatus if found, otherwise None.
        """
        agent = self.registry.get_agent(agent_name)
        return agent.status if agent else None

    def get_critical_agents(self) -> list[AgentMetadata]:
        """Identify all registered agents currently in an error or unreachable state.

        Returns:
            A list of AgentMetadata objects for agents with critical health status.
        """
        critical = []
    ...
    def get_status_summary(self) -> dict[str, int]:
        """Generate a high-level summary of the totals for each health status.

        Returns:
            A dictionary mapping status names to the count of agents in that state.
        """
        agents = self.registry.get_all_agents()
        return {
            "total": len(agents),
