...
def test_update_status_happy_path():
    ...
    r2 = client.patch(f"/api/v1/candidates/{candidate_id}/status", json={"status": "reviewing"}, headers=headers)
...
def test_update_status_unauthorized():
    # Create candidate
    payload = {
        "email": "unauth.status@example.com",
        "first_name": "NoAuth",
        "last_name": "User"
    }
    r = client.post("/api/v1/candidates", json=payload, headers={"Authorization": "Bearer test-token"})
    assert r.status_code == 201
...
def test_update_status_not_found():
    ...
    headers = {"Authorization": "Bearer test-token"}
    r = client.patch(f"/api/v1/candidates/{unknown_id}/status", json={"status": "reviewing"}, headers=headers)
