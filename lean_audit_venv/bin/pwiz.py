#!/home/asif1/open-talent/lean_audit_venv/bin/python3
...
DATABASE_MAP = {value: key for key in DATABASE_ALIASES for value in DATABASE_ALIASES[key]}


def make_introspector(database_type, database_name, **kwargs):
    if database_type not in DATABASE_MAP:
        err("Unrecognized database, must be one of: {}".format(", ".join(DATABASE_MAP.keys())))
        sys.exit(1)
    ...
    def _print_table(table, seen, accum=None):
        ...
        for name, column in columns:
            ...
            if is_unknown and ignore_unknown:
                disp = "{} - {}".format(column.name, column.raw_column_type or "?")
                print_(f"    # {disp}")
            else:
        ...
        if introspector.schema:
            print_(f"        schema = '{introspector.schema}'")
...
def print_header(cmd_line, introspector):
    ...
    print_("# Date: {}".format(timestamp.strftime("%B %d, %Y %I:%M%p")))
