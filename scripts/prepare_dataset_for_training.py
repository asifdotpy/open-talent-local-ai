...
def main():
    ...
    print(
        f"   Train: {len(train_examples)} examples ({len(train_examples) / len(examples) * 100:.1f}%)"
    )
    print(
        f"   Validation: {len(val_examples)} examples ({len(val_examples) / len(examples) * 100:.1f}%)"
    )
    ...
    for domain, count in sorted(stats.domains.items()):
        ...
        print(
            f"{domain:15s} Total: {count:4d} | Train: {train_count:4d} ({train_count / len(train_examples) * 100:5.1f}%) | Val: {val_count:3d} ({val_count / len(val_examples) * 100:5.1f}%)"
        )
    ...
    for diff, count in sorted(stats.difficulties.items()):
        ...
        print(
            f"{diff:15s} Total: {count:4d} | Train: {train_count:4d} ({train_count / len(train_examples) * 100:5.1f}%) | Val: {val_count:3d} ({val_count / len(val_examples) * 100:5.1f}%)"
        )
