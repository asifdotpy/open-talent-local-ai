...
class DatasetAnalyzer:
    def __init__(self, data_dir: str = "/home/asif1/talent-ai-platform/notebooks/data"):
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("# TalentAI Platform - Dataset Inventory Report\n")
        report.append(f"**Generated**: {datetime.datetime.now().strftime('%B %d, %Y at %H:%M UTC')}\n")
        report.append("---\n\n")
        ...
        report.append(f"- **Average Examples/Dataset**: {total_examples // max(len(self.datasets), 1)}\n\n")
        ...
        for name, examples in sorted(self.datasets.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            report.append(f"| {name} | {len(examples)} | {quality_str} | {categories} | {avg_length:.0f} | {status} |\n")
        ...
        if total_examples < 2000:
            report.append("- ⚠️ **Expand Data Coverage**: Current dataset is below optimal size. Target: 2000-3000 total examples across all agents.\n")
        
        if len(all_categories) < 15:
            report.append(f"- ⚠️ **Add Categories**: Only {len(all_categories)} categories covered. Target: 15+ diverse categories.\n")
        
        if len(all_domains) < 8:
            report.append(f"- ⚠️ **Add Domains**: Only {len(all_domains)} domains covered. Target: 8+ professional domains.\n")
        
        report.append("- 📋 **Next Steps**:\n")
        report.append("  1. Review `DATASET_STRATEGY_ROADMAP.md` for comprehensive plan\n")
        report.append("  2. Generate datasets for Scout, Boolean Mastery, Engagement, Market Intel, Tool Leverage, Quality, and Scanning agents\n")
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("TalentAI Dataset Inventory & Analysis Tool")
        print("="*60 + "\n")
