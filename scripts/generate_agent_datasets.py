...
Generates domain-specific datasets for all 8 agents in the TalentAI platform.
"""
...
class ScoutCoordinatorDatasetGenerator:
    ...
    def generate_examples() -> List[DatasetExample]:
        ...
        for i in range(50):
            workflow_stage = random.choice(['scanning', 'scoring', 'engagement', 'interviewing', 'closing'])
        ...
        for i in range(50):
            agents_involved = random.sample(['boolean_agent', 'scanning_agent', 'quality_agent', 'engagement_agent'], 2)
        ...
        for i in range(50):
            metric = random.choice(['success_rate', 'processing_time', 'quality_score', 'error_rate'])
        ...
        for i in range(40):
            ...
            if from_platform != to_platform:
                examples.append(DatasetExample(
                    ...
                    response=f"Query adapted successfully. Original: [source query]. Adapted: [target platform syntax]. Equivalent match probability: 0.95/1.0.",
...
class EngagementDatasetGenerator:
    ...
    def generate_examples() -> List[DatasetExample]:
        ...
        for i in range(50):
            achievement = random.choice(['open_source_contributions', 'speaking_engagements', 'patents', 'publications'])
            
            examples.append(DatasetExample(
                instruction=f"Craft LinkedIn connection request mentioning {achievement}",
                response=f"Hi [Name], I'm impressed by your {achievement} in the DevOps space. Would love to connect and discuss how we're solving similar problems at TalentAI. Looking forward!",
        ...
        for i in range(50):
            ...
            examples.append(DatasetExample(
                ...
                response=f"Hi! 👋 Quick check-in - are you still open to exploring that principal engineer role we discussed? We'd love to move forward this week. Let me know! 🚀",
        ...
        for i in range(50):
            stage = random.choice(['initial_outreach', 'after_one_week', 'after_two_weeks', 'final_followup'])
            ...
            examples.append(DatasetExample(
                ...
                domain="candidate_engagement",
                expected_length="medium",
                metadata={"stage": stage, "sequence_number": ["1", "2", "3", "4"][["initial_outreach", "after_one_week", "after_two_weeks", "final_followup"].index(stage)]}
            ))
    ...
    def generate_examples() -> List[DatasetExample]:
        ...
        for i in range(40):
            ...
            examples.append(DatasetExample(
                ...
                response=f"Competitive analysis: {competitor} strengths: [sourcing at scale], weaknesses: [limited AI avatar capabilities], TalentAI advantage: [avatar interviews + multi-agent sourcing], market position: [complementary to their sourcing-first approach]",
        ...
        # Market Insights (40)
        for i in range(40):
            insight_type = random.choice(['hiring_volume', 'skill_shortage', 'compensation_trends', 'remote_adoption'])
            
            examples.append(DatasetExample(
                instruction=f"Generate market insight report on: {insight_type}",
                response=f"Market insight - {insight_type}: [Quantified data], [Industry analysis], [Regional variations], [Forecast for next quarter], [Implications for TalentAI platform]",
...
class QualityScoringDatasetGenerator:
    ...
    def generate_examples() -> List[DatasetExample]:
        ...
        # Ranking Logic (50)
        for i in range(50):
            metric = random.choice(['skill_match', 'experience', 'cultural_fit', 'growth_potential', 'availability'])
            
            examples.append(DatasetExample(
                instruction=f"Rank 10 candidates using weighted scoring: skill_match=40%, experience=30%, cultural_fit=20%, growth={metric}=10%",
                response=f"Ranking calculation completed. Top candidate: [name], score=0.89/1.0 [breakdown]. 2nd: 0.82, 3rd: 0.78. Recommendations: Interview top-3. Pipeline depth: Strong.",
...
class ScanningDatasetGenerator:
    ...
    def generate_examples() -> List[DatasetExample]:
        ...
        # Data Extraction (40)
        for i in range(40):
            data_type = random.choice(['work_history', 'skills', 'projects', 'social_profiles', 'certifications'])
        ...
        for i in range(35):
            examples.append(DatasetExample(
                instruction=f"Deduplicate 500 profiles from multiple platform scans",
...
def main():
    ...
    print("\n" + "="*70)
    print("TalentAI Multi-Agent Dataset Generator")
    print("="*70 + "\n")
    
    base_dir = "/home/asif1/talent-ai-platform/notebooks/data"
    ...
    save_dataset(engagement_examples, f"{base_dir}/engagement_dataset.json", "Personalized Engagement")
    
    # Generate Market Intelligence dataset
    market_examples = MarketIntelligenceDatasetGenerator.generate_examples()
    save_dataset(market_examples, f"{base_dir}/market_intelligence_dataset.json", "Market Intelligence")
    ...
    print(f"\nNext steps:")
    print(f"  1. Run dataset validation: python scripts/validate_multi_dataset.py")
    print(f"  2. Generate inventory report: python scripts/audit_existing_datasets.py")
    print(f"  3. Upload to Hugging Face: python scripts/upload_all_datasets.py")
    print("\n")
