...
async def handle_scanning_request(message):
    ...
    try:
        ...
        sourcing_methods = message.payload.get("sourcing_methods", ["official_api", "opt_in_directory"])
...
async def scan_stackoverflow(
    ...
    logger.info(f"[STACK OVERFLOW] Successfully scanned with {len(compliance_records)} compliant records")
...
async def validate_scanning_request(request: ScanRequest):
    ...
    return {
        ...
        "warnings": [
            ...
            if len(validated_methods) < len(request.sourcing_methods) else None
        ],
        "recommendations": [
