"""
Boolean Mastery Agent - TalentAI Platform
...
async def handle_query_request(message):
    ...
    try:
        ...
        for platform in platforms:
            asyncio.create_task(
                generate_and_publish_query(pipeline_id, job_description, platform)
            )
    except Exception as e:
        logger.error(f"Error handling query request: {e}")


async def generate_and_publish_query(
    pipeline_id: str,
    job_description: str,
    platform: str
):
