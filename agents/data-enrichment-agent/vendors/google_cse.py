...
class GoogleCSEClient:
    ...
    async def test_connection(self):
        ...
        async with self.session.get(self.BASE_URL, params=params) as response:
            ...
            logger.info(f"Google CSE test successful. Quota: {self.daily_quota_limit - self.daily_quota_used} remaining")
        ...
        try:
            async with self.session.get(self.BASE_URL, params=params) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Google CSE: Query '{query}' returned {len(results.get('items', []))} results")
                    return results
