...
async def enrich_via_stackoverflow(query: str) -> Dict:
    ...
    try:
        ...
        search_url = f"{base_url}/users?order=desc&sort=reputation&inname={query}&site=stackoverflow"
            ...
            try:
                ...
                if linkedin_links:
                    ...
                    if profile_url:
                        ...
                        name_element = await page.query_selector('h1.text-heading-xlarge, h1[data-test-id="hero__page__title"]')
                        headline_element = await page.query_selector('.text-body-medium, .pv-text-details__left-panel .text-body-medium')
                        location_element = await page.query_selector('.text-body-small.inline.t-black--light.break-words, .pv-text-details__left-panel .text-body-small')

                        full_name = await name_element.text_content() if name_element else name
                        headline = await headline_element.text_content() if headline_element else None
                        location = await location_element.text_content() if location_element else location
...
async def candidate_search_duckduckgo(query: str, max_results: int = 5) -> List[Dict]:
    ...
    try:
        ...
        with DDGS() as ddgs:
            ...
            linkedin_results = list(ddgs.text(linkedin_query, max_results=max_results*2))  # Get more results to filter

            logger.info(f"DuckDuckGo LinkedIn search: '{linkedin_query}' found {len(linkedin_results)} results")
                    ...
                    if 'linkedin.com/in/' in href or 'linkedin.com/pub/' in href:
                        ...
                        if not name or name in ['LinkedIn', 'Python Developer']:
                            ...
                            if 'linkedin.com/in/' in href:
                                path_parts = href.split('linkedin.com/in/')[1].split('/')[0].split('?')[0]
            ...
            logger.info(f"DuckDuckGo GitHub search: '{github_query}' found {len(github_results)} results")
        ...
        logger.info(f"✅ DuckDuckGo candidate search: Found {len(candidates)} candidates for '{query}'")
        return candidates
...
async def candidate_search_combined(query: str, location: str = None, skills: str = None, max_results: int = 10) -> List[Dict]:
    """
    ...
    try:
        ...
        if DUCKDUCKGO_AVAILABLE and False:  # Temporarily disabled
            try:
                ddg_candidates = await candidate_search_duckduckgo(full_query, max_results=max_results//2)
        ...
        try:
            ...
            skills_keywords = ['python', 'django', 'react', 'javascript', 'java', 'c++', 'c#', 'php', 'ruby', 'go', 'rust',
                              'tensorflow', 'pytorch', 'machine learning', 'ml', 'ai', 'data science', 'web development']
        ...
        logger.info(f"✅ Combined candidate search: Found {len(unique_candidates)} unique candidates for '{full_query}'")
...
async def enrich_profile_auto(url: str, user_id: str) -> Dict:
    ...
    if 'github.com' in url:
        try:
            ...
            log_enrichment(user_id, url, SourceMethod.GITHUB_PUBLIC, success=True, data_extracted=True)
            return profile
        except Exception as e:
    ...
    if 'stackoverflow.com' in url:
        try:
            ...
            log_enrichment(user_id, url, SourceMethod.STACKOVERFLOW, success=True, data_extracted=True)
            return profile
        except Exception as e:
...
class CandidateSearchPipeline:
    ...
    async def update_status(self, new_status: PipelineStatus, message: str = None):
        ...
        logger.info(f"📊 Pipeline {self.request_id}: {self.status.value} - {len(self.results['candidates'])} candidates")

    async def phase_sourcing(self, query: str, location: str = None, skills: str = None, max_results: int = 10):
        ...
        try:
            await asyncio.wait_for(asyncio.gather(*sourcing_tasks, return_exceptions=True), timeout=30.0)
        except asyncio.TimeoutError:
            logger.warning(f"Pipeline {self.request_id}: Sourcing timeout, proceeding with partial results")

        # Mark sourcing complete
        self.progress["sourcing_complete"] = True
        await self.update_status(PipelineStatus.SOURCING, f"Sourcing complete: {len(self.results['candidates'])} candidates found")
    ...
    async def phase_enrichment(self):
        ...
        if enrichment_tasks:
            try:
                await asyncio.wait_for(asyncio.gather(*enrichment_tasks, return_exceptions=True), timeout=20.0)
            except asyncio.TimeoutError:
                logger.warning(f"Pipeline {self.request_id}: Enrichment timeout")

        self.progress["enrichment_complete"] = True
        await self.update_status(PipelineStatus.ENRICHING, f"Enrichment complete: {len(self.results['candidates'])} candidates enriched")
    ...
    async def _enrich_candidate(self, candidate: Dict, index: int):
        ...
        try:
            ...
            enrichment_url = (candidate.get('linkedin_url') or
                            candidate.get('github_url') or
                            candidate.get('stackoverflow_url'))
            ...
            if enrichment_url:
                ...
                if enriched_data:
                    ...
                    candidate.update({
                        ...
                        "enrichment_timestamp": datetime.now().isoformat(),
                        "accuracy_score": min(1.0, candidate.get('accuracy_score', 0) + 0.2)  # Boost accuracy
                    })
    ...
    async def phase_presentation(self, query: str, location: str = None, skills: str = None):
        ...
        await self.update_status(PipelineStatus.COMPLETED, f"Search complete: {len(self.results['candidates'])} unique candidates found")

    async def execute(self, query: str, location: str = None, skills: str = None, max_results: int = 10):
    ...
    async def _publish_final_results(self, query: str, location: str, skills: str):
        ...
        logger.info(f"✅ Pipeline {self.request_id} complete: {len(self.results['candidates'])} candidates found in {(self.end_time - self.start_time).total_seconds():.1f}s")
...
async def handle_enrichment_request(message: Dict):
    ...
    logger.info(f"✅ Enrichment complete: {len(enriched_profiles)} profiles for pipeline {pipeline_id}")
...
async def handle_candidate_search_request(message: Dict):
    ...
    logger.info(f"🔍 Starting candidate search pipeline: '{query}' (location: {location}, skills: {skills})")
...
async def lifespan(app: FastAPI):
    ...
    await message_bus.subscribe(
        topics=["agents:enrichment"],
        callback=handle_enrichment_request
    )
...
async def health_check():
    ...
    candidate_search_status = "✅ Ready" if (PLAYWRIGHT_AVAILABLE and DUCKDUCKGO_AVAILABLE) else "⚠️ Limited"
...
class CandidateSearchRequest(BaseModel):
    ...
    skills: Optional[str] = Field(None, description="Skills filter")
    ...
    return {
        "free_tier": [
            ...
            },
            {
                ...
                "features": ["Concurrent multi-source search", "Real-time progress tracking", "Automatic deduplication", "Profile enrichment"],
                "legal": "✅ GDPR compliant"
            }
        ],
        "paid_tier": [
...
async def cache_stats():
    """Cache statistics"""
    return {
        "cached_profiles": len(profile_cache),
...
async def get_pipeline_status(request_id: str):
    ...
    return {
        ...
        "estimated_completion_seconds": max(0, 30 - (datetime.now() - pipeline.start_time).total_seconds())
    }
