"""
Tool Leverage Agent - TalentAI Platform
...
async def lifespan(app: FastAPI):
    ...
    await message_bus.subscribe(
        ["agents:tools", Topics.CANDIDATE_EVENTS],
        handle_sync_request
    )
...
async def handle_sync_request(message):
    ...
    try:
        if message.message_type == MessageType.CANDIDATE_FOUND:
            ...
            asyncio.create_task(
                sync_candidate(candidate_id, target_system)
            )
