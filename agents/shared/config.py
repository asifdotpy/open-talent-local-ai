"""
Shared configuration for TalentAI agents.
...
class AgentConfig(BaseSettings):
    ...
    neo4j_uri: str = Field(
        default="neo4j+s://4b63e239.databases.neo4j.io",
        env="NEO4J_URI"
    )
    ...
    conversation_service_url: str = Field(
        ...
        env="CONVERSATION_SERVICE_URL"
    )
    voice_service_url: str = Field(
        default="http://localhost:8002",
        env="VOICE_SERVICE_URL"
    )
    avatar_service_url: str = Field(
        default="http://localhost:8001",
        env="AVATAR_SERVICE_URL"
    )
    interview_service_url: str = Field(
        default="http://localhost:8004",
        env="INTERVIEW_SERVICE_URL"
    )
    genkit_service_url: str = Field(
        default="http://localhost:3400",
        env="GENKIT_SERVICE_URL"
    )
    
    # Agent URLs
    scout_agent_url: str = Field(
        default="http://localhost:8090",
        env="SCOUT_AGENT_URL"
    )
    vetta_agent_url: str = Field(
