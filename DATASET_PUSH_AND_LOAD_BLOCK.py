...
if local_train.exists() and local_val.exists():
    ...
    try:
        ...
        print(
            f"🔗 View at: https://huggingface.co/datasets/asifdotpy/{config['huggingface']['dataset_repo'].split('/')[-1]}"
        )

    except Exception as e:
        print(f"❌ Push failed: {e}")
        print("\n🔧 Troubleshooting:")
        print("   1. Check HF_TOKEN is valid: from huggingface_hub import login; login()")
        print("   2. Ensure token has 'datasets' write permission")
        print("   3. Check if dataset repo already exists (can overwrite with existing)")
...
# ─────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        print("📋 Next steps:")
        print("   1. Push dataset from local machine: python scripts/push_dataset_to_hub.py")
        print(
            f"   2. Or manually run: dataset_dict.push_to_hub('{config['huggingface']['dataset_repo']}')"
        )
...
val_dataset = (
    dataset_dict["validation"].map(format_example) if "validation" in dataset_dict else None
)
