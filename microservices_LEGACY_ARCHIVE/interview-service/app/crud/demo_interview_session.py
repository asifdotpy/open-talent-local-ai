...
def create_demo_session(
    db: Session, session_data: DemoInterviewSessionCreate
...
def get_demo_session_by_id(db: Session, session_id: str) -> Optional[DemoInterviewSession]:
    ...
    """
    return (
        db.query(DemoInterviewSession).filter(DemoInterviewSession.session_id == session_id).first()
    )
...
def update_demo_session_status(
    db: Session, session_id: str, status: str
) -> Optional[DemoInterviewSession]:
    """
