...
from app.services import audit_service, conversation_service, avatar_service
...
def create_demo_interview_session(
    *, db: Session = Depends(deps.get_db), payload: DemoInterviewSessionCreate
):
    """
...
def start_interview(*, db: Session = Depends(deps.get_db), payload: HandoffPayload) -> dict:
    ...
    first_dialogue = conversation_service.initiate_conversation(
