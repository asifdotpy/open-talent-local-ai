...
async def api_docs_info():
    ...
    return {
        ...
        "documentation_urls": {"swagger_ui": "/docs", "redoc": "/redoc", "openapi_json": "/openapi.json"},
        "routes": routes_info,
    }
    ...
    if vector_db is not None:
        ...
        for cand_id, cand in list(candidates_db.items())[:limit]:
            ...
            if query.lower() in full_name or query.lower() in cand.get("email", "").lower():
                results.append(
                    {"id": cand_id, "full_name": full_name.strip(), "email": cand.get("email"), "score": 1.0}
                )
...
@app.get(
    ...
    responses={200: {"description": "Paginated list of candidates"}, 403: {"description": "Forbidden"}},
)
...
@app.get(
    ...
    responses={200: {"description": "Paginated list of applications"}, 403: {"description": "Forbidden"}},
)
...
async def get_candidate_applications(candidate_id: str, current_user: str | None = Depends(get_current_user)):
    ...
    candidate_apps = [app for app in applications_db.values() if app["candidate_id"] == candidate_id]
...
async def create_interview(
    candidate_id: str, payload: InterviewCreate, current_user: str | None = Depends(get_current_user)
):
    if not current_user:
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})
    if candidate_id not in candidates_db:
...
async def update_interview(
    candidate_id: str, interview_id: str, payload: InterviewUpdate, current_user: str | None = Depends(get_current_user)
):
    if candidate_id not in candidates_db:
        return JSONResponse(status_code=404, content={"error": "Candidate not found"})
    interviews = interviews_db.get(candidate_id, [])
...
async def create_assessment(
    candidate_id: str, payload: AssessmentCreate, current_user: str | None = Depends(get_current_user)
):
    if not current_user:
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})
    if candidate_id not in candidates_db:
...
async def create_availability(
    candidate_id: str, payload: AvailabilityCreate, current_user: str | None = Depends(get_current_user)
):
    if not current_user:
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})
    if candidate_id not in candidates_db:
    ...
    if vector_db is None:
        ...
        if candidate_id in mock_candidate_ids:
            mock_candidate_profile = CandidateProfile(
                ...
                skills=Skills(matched=["Python", "FastAPI", "SQLAlchemy"], unmatched=["React", "TypeScript"]),
