...
async def get_user_preferences(
    ...
    if user_id not in user_preferences_db:
        return JSONResponse(status_code=404, content={"error": "Preferences not found"})

    return JSONResponse(status_code=200, content=user_preferences_db[user_id])
...
async def get_current_user_preferences(current_user: Optional[str] = Depends(get_current_user)):
    ...
    return JSONResponse(status_code=200, content=user_preferences_db[current_user])
...
async def get_user_emails(user_id: str, current_user: Optional[str] = Depends(get_current_user)):
    ...
    return JSONResponse(status_code=200, content={"emails": user_emails_db[user_id]})
...
async def remove_email(
    ...
    return JSONResponse(status_code=204, content={"message": "Email removed"})
...
async def get_user_phone_numbers(
    ...
    return JSONResponse(status_code=200, content={"phones": user_phones_db[user_id]})
...
async def remove_phone_number(
    ...
    return JSONResponse(status_code=204, content={"message": "Phone removed"})
...
async def get_user_activity_log(
    ...
    return JSONResponse(status_code=200, content={"activity": user_activity_db[user_id]})
...
async def get_user_sessions(user_id: str, current_user: Optional[str] = Depends(get_current_user)):
    ...
    return JSONResponse(status_code=200, content={"sessions": user_sessions_db[user_id]})
...
async def revoke_user_session(
    ...
    user_sessions_db[user_id] = [
        s for s in user_sessions_db[user_id] if s.get("session_id") != session_id
    ]

    return JSONResponse(status_code=204, content={"message": "Session revoked"})
