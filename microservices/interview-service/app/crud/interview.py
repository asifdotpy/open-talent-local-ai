...
def _create_search_criteria(db: Session, search_criteria_payload: interview_schemas.SearchCriteria) -> interview_models.SearchCriteria:
    """
...
def _create_candidate_profile(db: Session, candidate_profile_payload: interview_schemas.CandidateProfile) -> interview_models.CandidateProfile:
    """
...
def _create_work_experiences(db: Session, candidate_id: int, work_experiences_payload: list[interview_schemas.WorkExperience]):
    """
    ...
    for exp in work_experiences_payload:
        ...
        work_exp = interview_models.WorkExperience(
            **data, 
...
def _create_education_records(db: Session, candidate_id: int, education_payload: list[interview_schemas.Education]):
    """
...
def _associate_skills(db: Session, candidate_profile: interview_models.CandidateProfile, skills_payload: interview_schemas.Skills):
    """
    Creates and associates Skill records with a candidate profile.
    """
    for skill_name in skills_payload.matched:
        skill = db.query(interview_models.Skill).filter(interview_models.Skill.name == skill_name).first()
        if not skill:
...
def _create_initial_questions(db: Session, candidate_id: int, initial_questions_payload: list[interview_schemas.InitialQuestion]):
    """
...
def _create_interview_record(db: Session, search_criteria_id: int, candidate_profile_id: int) -> interview_models.Interview:
    """
...
def create_interview(db: Session, payload: interview_schemas.HandoffPayload) -> interview_models.Interview:
    """
