#!/usr/bin/env python3
"""
Cross-service integration tests for OpenTalent microservices.
...
class IntegrationTestSuite:
    """Integration test suite for all OpenTalent services."""
    ...
    async def test_all_services_running(self):
        ...
        optional_services = [("Frontend Service", f"{FRONTEND_SERVICE_URL}/")]
    ...
    async def test_end_to_end_interview_flow(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            room_response = await client.post(
                f"{INTERVIEW_SERVICE_URL}/create-interview-room", json=room_payload, timeout=TIMEOUT
            )
            ...
            assert "OpenTalent Interview" in jitsi_response.text

            print("✅ Custom Jitsi interface accessible")

            # Step 3: Test Avatar service voice generation (mock implementation)
            voice_payload = {
                "text": "Welcome to your interview, integration test candidate! I'm Interview, and I'll be conducting your interview today.",
                "voice_id": "mock_voice",
            }

            voice_response = await client.post(
                f"{AVATAR_SERVICE_URL}/api/v1/generate-voice", json=voice_payload, timeout=TIMEOUT
            )
        ...
        async def create_concurrent_room(candidate_id: str):
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    ...
                    json={
                        "candidateId": f"load-test-{candidate_id}",
                        "interviewerId": "load-interviewer",
                    },
                    timeout=TIMEOUT,
                )
                end_time = time.time()
...
async def run_integration_tests():
    ...
    print("🧪 Starting OpenTalent Integration Test Suite")
    print("=" * 60)
