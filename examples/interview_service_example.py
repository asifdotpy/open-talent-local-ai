...
# Import Vetta AI
import sys
sys.path.append('/home/asif1/talent-ai-platform')
from microservices.shared.vetta_service import VettaService

# Initialize FastAPI app
app = FastAPI(title="TalentAI Interview Service", version="4.0.0")
...
async def root():
    """Health check"""
    return {
        "service": "TalentAI Interview Service",
        "version": "4.0.0",
...
async def assess_candidate(request: CandidateAssessment):
    ...
    try:
        # Build instruction
        instruction = "Assess this candidate's fit for the role and provide comprehensive evaluation"
        ...
        return InterviewResponse(
            ...
            metadata={
                ...
                "num_responses": len(request.interview_responses) if request.interview_responses else 0
            }
        )
